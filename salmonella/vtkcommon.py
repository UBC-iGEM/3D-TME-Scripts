...
def vtkGetDataSetBounds(dataset, mode='[xyz],[xyz]'):
    ...
    if mode == '[xyz],[xyz]':
        ...
        raise RuntimeError('vtkGetDataSetBounds does not know mode %s'
                           % mode)
...
def copyPolyDataCells(dataset, cellids):
    ...
    result.GetCellData().CopyAllocate(dataset.GetCellData(), 0, 0,
            False)
    result.GetPointData().CopyAllocate(dataset.GetPointData(), 0, 0,
            False)
...
def vtkImageDataToNumpy(dataset, datanames=None):
    assert isinstance(dataset, vtk.vtkImageData), \
...
def vtkImageDataAddData(
    ...
    d = (grid.GetCellData() if at == 'CellData'
          else grid.GetPointData())
    data = np.atleast_3d(data)
    if len(data.shape) == 4 and data.shape[3] == dim:
        data = np.asarray(data).transpose().reshape((dim,
                -1)).transpose()
...
def vtkIntegrateData(dataset):
    ...
    class ValueHandler:
        ...
        def __init__(self, fd):
            self.fd = fd
            self.fd_arrays = [fd.GetArray(i) for i in
                              xrange(fd.GetNumberOfArrays())]
            self.value = np.zeros(sum(a.GetNumberOfComponents()
...
#        weight += math.sqrt(np.sum(np.square(y-x)))
    ...
    return (cd_handler.get_values(), pd_handler.get_values(),
            weight_sum)
...
def fromVtkTriangleMesh(vtkds):
  ...
  """
    ...
    for cell in vtkIterCells(vtkds):
        ...
        triangles.append(t)
    points = [vtkds.GetPoint(i) for i in
              xrange(vtkds.GetNumberOfPoints())]
...
def vtkGetLineComponents(surf):
  ...
  # pprint.pprint(components)
    ...
    points = np.asarray([surf.GetPoint(j)[:2] for j in
                        xrange(num_points)])
...
def ZippedRead(Reader, fn):
    ...
    if fn.endswith('.gz'):
        ...
        f = gzip.open(fn, 'r')
        (fd, fn2) = tempfile.mkstemp(suffix='.vtu',
                dir=os.path.dirname(fn), text=True)
...
def vtkDatasetFromHdf5(g):
  ...
  """
    ...
    gridtype2vtureader = {'vtkImageData': vtk.vtkXMLImageDataReader,
...
def vtkDatasetToHdf5(g, name, ds):
  ...
  # assert type(ds) is vtk.vtkPolyData
    ...
    ds = g.create_dataset(name, data=np.fromstring(iso,
...
def vtkRender2d(
  ...
  # actor.GetProperty().SetRepresentationToWireframe()
    ...
    c.SetClippingRange(c.GetPosition()[2] * 0.9, c.GetPosition()[2]
                       * 1.1)
